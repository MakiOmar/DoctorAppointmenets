...
function display_sales_report( $atts ) {
	...
	foreach ( $products as $product ) {
		...
		$pending_sales *= ANONY_ADMIN_FEES_FACTOR; // Subtract 10% from pending sales
	...
	$output  = '<table>';
	$output .= '<tr><th>اسم المنتج</th><th>الكمية المباعة</th><th>المبيعات المعلقة</th><th>إجمالي المبيعات</th></tr>'; // Change column order
...
function anony_display_available_withdrawal_credit( $user ) {
	if ( current_user_can( 'manage_options' ) ) {
		...
		?>
		<h3><?php _e( 'Available Withdrawal Credit', 'textdomain' ); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="available_withdrawal_credit"><?php _e( 'Credit', 'textdomain' ); ?></label></th>
				<td>
	...
	if ( ! empty( $completed_orders ) ) {
		foreach ( $completed_orders as $completed_order ) {
			...
			if ( 'cod' == $order_payment_method ) {
				$cerdit += $completed_order->get_total();
			}
		}
	}
	$cerdit *= ANONY_ADMIN_FEES_FACTOR;
	return $cerdit;
}
function anony_current_user_sales_orders_vertical( $atts ) {
	$current_user_id = get_current_user_id();

	$withdrawal_amout = anony_available_withdrawal_credit();

	if ( $withdrawal_amout != 0 ) {
		ob_start();
		?>
		<h3>رصيد السحب المتاح: <span style="color:#3A9A38;font-weight:bold"><?php echo wc_price( $withdrawal_amout ); ?></span></h3>
	...
	/*
	echo '<pre>';
	var_export(count($completed_orders));
	echo '</pre>';*/
	// Get processing orders
	$processing_orders = anony_get_vendor_sales_orders( 'wc-processing' );

	// Generate completed orders tables
	$completed_orders_tables    = '<style>/* Remove list styling and create inline list items */
        ...
        /* Set active tab to have green background */
        .anony-sales-order-tab-links li.active a, .set-as-completed {
	...
	if ( ! empty( $completed_orders ) ) {
		foreach ( $completed_orders as $completed_order ) {
			...
			$completed_orders_table .= '<tr><th>طريقة الدفع او السداد:</th><td>' . esc_html( $order_payment_method_title ) . '</td></tr>';
			...
			$completed_orders_table .= '<tr><th></th><td><a href="' . site_url( '/order-details/?order_id=' . $order_id ) . '" class="button">' . esc_html__( 'View', 'text-domain' ) . '</a></td></tr>';
		...
		$completed_orders_tables .= '<tr>' . esc_html__( 'لم يتم الانتهاء من الطلبات بعد ...' ) . '</tr>';
		$completed_orders_tables .= '</table>';
	}

	// Generate processing orders tables
	$processing_orders_tables = '';
	...
	if ( ! empty( $processing_orders ) ) {
		foreach ( $processing_orders as $processing_order ) {
			...
			if ( $processing_order->get_payment_method() === 'cod' ) {
				$as_completed = ' <a href="#" data-id="' . $order_id . '" class="set-as-completed">وضع علامة كمكتمل</a>';
			}

			if ( empty( $order_payment_method_title ) ) {
				$order_payment_method_title = $processing_order->get_payment_method();
			}
			...
			$processing_orders_table .= '<tr><th>طريقة الدفع او السداد:</th><td>' . esc_html( $order_payment_method_title ) . '</td></tr>';
			...
			$processing_orders_table .= '<tr><th></th><td class="action-buttons"><a href="' . site_url( '/order-details/?order_id=' . $order_id ) . '" class="button">' . esc_html__( 'View', 'text-domain' ) . '</a>' . $as_completed . '</td></tr>';
			$processing_orders_table .= '</table>';

		}

		$processing_orders_tables .= $processing_orders_table;
	} else {
		$processing_orders_tables .= '<table class="processing-order">';
		$processing_orders_tables .= '<tr>' . esc_html__( 'No processing orders yet...' ) . '</tr>';
		$processing_orders_tables .= '</table>';
	}
	...
	$tabs .= '<div id="processing-orders" class="anony-sales-order-tab-pane">' . $processing_orders_tables . '</div>';
	$tabs .= '</div>';
	$tabs .= '</div>';
...
add_shortcode(
	...
	function () {
		...
		$order_id   = $_GET['order_id']; // Replace with the ID of the order you want to display
			...
			<table class="table">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $order_data['date_created']->format( 'Y-m-d H:i:s' ); ?></td>
					</tr>
					<tr>
						<th>بيانات الفاتورة</th>
						<td>
							<?php echo $order_data['billing']['first_name']; ?> <?php echo $order_data['billing']['last_name']; ?><br>
							<?php echo $order_data['billing']['email']; ?><br>
					...
					<tr>
						<th>بيانات الشحن</th>
						<td>
							<?php echo $order_data['shipping']['first_name']; ?> <?php echo $order_data['shipping']['last_name']; ?><br>
							<?php echo $order_data['shipping']['address_1']; ?><br>
							<?php echo $order_data['shipping']['address_2']; ?><br>
							<?php echo $order_data['shipping']['city']; ?>, <?php echo $order_data['shipping']['state']; ?> <?php echo $order_data['shipping']['postcode']; ?><br>
							<?php echo $order_data['shipping']['country']; ?>
						</td>
					</tr>
					<tr>
						<th>المنتجات</th>
...
function save_order_product_author_ids( $order_id ) {
	...
	foreach ( $items as $item ) {
		...
		if ( $product ) {
			...
			if ( ! in_array( $product_author_id, $product_author_ids ) ) {
...
add_action(
	'wp_footer',
	function () {
		// error_log(print_r( anony_get_vendor_sales_orders( 'wc-processing' ), true ));
	}
);
add_filter(
	'logout_redirect',
...
function anony_display_custom_data_after_billing( $order ) {
	...
	echo '<p><strong>' . __( 'Tota savings:', 'woocommerce' ) . '</strong> ' . $savings . '</p>';
	echo '</div>';
}
...
function anony_display_order_total_savings( $order ) {
	...
	if ( $total_savings > 0 ) {
		echo '<p><strong>Total Savings:</strong> ' . wc_price( $total_savings ) . '</p>';
	}
}
...
function anony_display_total_savings_form() {
	if ( is_wc_endpoint_url( 'orders' ) ) {
		$selected_savings_month = isset( $_POST['savings_month'] ) ? sanitize_text_field( $_POST['savings_month'] ) : false;
		$selected_savings_year  = isset( $_POST['savings_year'] ) ? sanitize_text_field( $_POST['savings_year'] ) : false;
		?>
		<form method="post" action="">
			<label for="savings_month"> اختر الشهر </label>
			<select name="savings_month" id="savings_month">
				<?php
				for ( $i = 1; $i <= 12; $i++ ) {
					$savings_month = date( 'F', mktime( 0, 0, 0, $i, 1 ) );
					echo '<option value="' . $savings_month . '" ' . selected( $selected_savings_month, $savings_month, false ) . '>' . $savings_month . '</option>';
				}
				?>
			</select>
			<label for="savings_year">اختر السنة</label>
			<select name="savings_year" id="savings_year">
				<?php
				$current_savings_year = date( 'Y' );
				for ( $i = $current_savings_year; $i >= 2020; $i-- ) {
					echo '<option value="' . $i . '" ' . selected( $selected_savings_year, $i, false ) . '>' . $i . '</option>';
				}
				?>
			</select>
			<input type="submit" name="submit" value="عرض جميع التوفيرات">
		</form>
		<?php
	}
}
...
function anony_display_total_savings_for_customer_orders( $user_id = false ) {
	...
	if ( is_wc_endpoint_url( 'orders' ) || strpos( $_SERVER['REQUEST_URI'], 'orders/total-savings' ) !== false ) {
		...
		if ( $selected_savings_month && $selected_savings_year ) {
			$first_day_of_savings_month = date( 'Y-m-01', strtotime( $selected_savings_month . ' ' . $selected_savings_year ) );
			$last_day_of_savings_month  = date( 'Y-m-t', strtotime( $first_day_of_savings_month ) );

			$args['date_created'] = $first_day_of_savings_month . '...' . $last_day_of_savings_month;

			$result_msg = 'Total Savings for all orders of ' . $selected_savings_month . ' ' . $selected_savings_year . ' is: ';
		}
		...
		if ( strpos( $_SERVER['REQUEST_URI'], 'orders/total-savings' ) !== false && ( ! $orders || empty( $orders ) ) ) {
			...
			$response = array(
				...
				'content'   => array(
					'total_savings' => 0,
					'msg'           => 'No completed or delivered orders found!',
				),
			);
			return $response;

		} elseif ( strpos( $_SERVER['REQUEST_URI'], 'orders/total-savings' ) === false && ( ! $orders || empty( $orders ) ) ) {
			echo 'No completed or delivered orders found!';
...
function anony_send_total_savings_for_customer_orders( $request ) {
	$customer_id = $request->get_param( 'customer_id' );
		// return array( 'total_savings' => $customer_id );
...
function anony_validate_cart_author( $passed, $product_id, $quantity ) {
	...
	foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
		...
		if ( $cart_item_author_id !== $product_author_id ) {
			...
			wc_add_notice( __( 'لا يمكنك إضافة منتجات من بائع مختلف إلى سلة التسوق.', 'your-text-domain' ), 'خطأ' );

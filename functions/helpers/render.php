	...
	if ( is_array( $timetables ) ) {
		...
		foreach ( $day_groups as $date => $timetables ) {
			...
			$output     .= '<div class="anony-grid-row snks-bg snks-timetable-accordion" data-id="' . $date . '">';
				...
				$output .= '</div>';
			$output     .= '</div>';
			$output     .= '<div style="background-color:#dcdcdc;" class="anony-padding-10 anony-flex flex-h-center flex-v-center">';
			$output     .= '<button class="anony-padding-5 snks-bg">تعديل</button>';
			$output     .= '</div>';

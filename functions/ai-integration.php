...
class SNKS_AI_Integration {
	...
	public function chat_diagnosis_ajax() {
		...
		// Skip message validation if conversation is empty
		if ( ! $is_conversation_empty ) {
			// Only validate message if conversation is not empty
...
%s Team',
	...
	private function process_chat_diagnosis( $message, $conversation_history, $user_id = null, $conversation_id = '', $locale = '' ) {
		...
		$refusal_patterns = array(
			...
			'cannot start without finishing'
		);

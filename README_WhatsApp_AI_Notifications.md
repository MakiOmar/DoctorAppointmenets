...
{{patient_name}} = patient name, {{day_name}} = day name, {{date}} = date, {{time}} = time
```
تم حجز جلسة جديدة عبر ( موقع جلسة أونلاين ).
إسم العميل: {{patient_name}}
اليوم: {{day_name}}
الموافق: {{date}}
الساعة: {{time}} بتوقيت مصر
```
...
```
أهلا {{patient_name}}! 
قام المعالج الخاص بك ( {{doctor_name}} ) بتفعيل خدمة روشتة لك، يرجى تحديد موعد الجلسه عن طريق صفحة الحجوزات خلال أسبوع من الآن لتفادي إلغاء الخدمة.
```

### 4. rosheta_app (3 parameters)
{{day_name}} = day name, {{date}} = date, {{time}} = time
```
تم حجز جلسة مع الطبيب النفسي الأخصائي الخاص بالموقع يوم {{day_name}} الموافق {{date}} الساعة {{time}} بتوقيت مصر، ويمكنك الدخول للجلسة في موعدها من خلال صفحة الحجوزات بحسابك على موقع جلسة.
...
{{doctor_name}} = doctor name, {{day_name}} = day name, {{date}} = date, {{time}} = time
```
نذكرك بموعد جلستك غدا {{day_name}} الموافق {{date}} على موقع جلسة مع المعالج {{doctor_name}} الساعة {{time}} بتوقيت مصر، ويمكنك الدخول للجلسة في موعدها من خلال صفحة الحجوزات بحسابك على الموقع، وسيتم عمل الجلسة عبر نظام الاتصالات الخاص بالموقع.
...
{{day_name}} = day name, {{date}} = date
```
نذكرك بجلساتك غدا {{day_name}} الموافق {{date}} المحجوزه عبر ( موقع جلسة أونلاين )، يرجى التأكد من التواجد والدخول لكل جلسة في موعدها
```
